...
**終了コードは失敗の分類にだけ使い、合否の条件は変えていません。** 例えば
課題1では、エラーが出ることを期待する入力で異常終了すると今も通ります
(`.stderr` に何か出ていれば通る判定のため)。それを落とすのは採点の変更で、
結果の見やすさの改修とは別の決定です。
...
合否を変えていないことは、改修の前後で同じ課題ディレクトリに対して
`pytest -q --tb=no -rA` を走らせ、`PASSED`/`FAILED` の一覧を突き合わせて
確かめます。
